...
import com.projeto.exception.InvalidJwtAuthenticationException;
...
public class JwtTokenProvider {
	...
	private String getAcessToken(String username, List<String> roles, Date now, Date validity) {
		Date validityRefreshToken = new Date(now.getTime() + (validityInMilliseconds * 3)); //Três horas
	...
	public Authentication getAuthentication(String token) {
		...
		UserDetails userDetails = this.userDatailService.loadUserByUsername(decodedJWT.getSubject());
	...
	//valida o token quando a pessoa estiver autenticando na aplicação, verificar se o token é valido
